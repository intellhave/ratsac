...
namespace ToolBox{
  ...
  QuasiconvexFit::QuasiconvexFit(const Eigen::MatrixXd &u1_, const Eigen::MatrixXd &u2_, std::vector<int>* sorted_, Config *cfg_){
    u1 = u1_;
    u2 = u2_;
    cfg = cfg_;
    sorted = sorted_;
  ...
  void QuasiconvexFit::genMatrixQuasiconvex(){
  ...
  void QuasiconvexFit::genMatrixAffine(){
    A.resize(N, 6);
    b.resize(2, nPoints);
    c.resize(6, nPoints);
    d.resize(nPoints);
    ...
    for (int i=0; i<nPoints; i++){
      ...
      Eigen::VectorXd ai1(6), ai2(6), ci(6);
      ai1 << x1, x2, 1, 0, 0, 0;
      ai2 << 0, 0, 0, x1, x2, 1;
      A.row(2*i) = -ai1;
      A.row(2*i+1) = -ai2;
  ...
  void QuasiconvexFit::genMatrixHomography(){
    A.resize(N, 8);
    b.resize(2, nPoints);
    c.resize(8, nPoints);
    d.resize(nPoints);
  ...
  void QuasiconvexFit::ransacFit(){
    if (cfg->problem==HOMOGRAPHY)
      homoRansacFit();
    else
      affineRansacFit();
  }

  void QuasiconvexFit::affineRansacFit(){
    AffineEstimator sac;
    double th_temp;
    ...
    genMatrixAffine();
    sac.init_problem(&A, &b, &c, &d);

    sac.solve();
    print_usac_results(sac.results_);
    
    Eigen::VectorXd ransacTheta(6);
    for (int i=0; i<6; i++)
      ransacTheta[i] = sac.final_model_params_[i];
  ...
  void QuasiconvexFit::homoRansacFit(){
    vector<double> pts;
    for (int i=0; i<nPoints; i++)
    {
  ...
  void QuasiconvexFit::Fit(const string &method)
  {
    startTime = clock();
