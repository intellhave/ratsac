...
namespace FTools
{
void normalizePoints(double* inputPointsUnNorm, double* inputPoints, int numPoints, double* T1, double* T2)
{
  for (int i = 0; i < 3; ++i) {
    for (int j = 0; j < 3; ++j) {
      T1[3*i+j] = 0;
...
void formCovMat(double* Cv, const double* Z, int len, int siz)
{
  int lenM = len*siz;
  double val;
...
int getHError(std::vector<double>& errs, double* u, int nr, double* H, double threshold, const std::vector<int> *test)
{
  double* model = H;
  double inv_model[9];
  ...
  if (test != NULL)
    test_nr = test->size();
  errs.clear(); errs.resize(test_nr);
